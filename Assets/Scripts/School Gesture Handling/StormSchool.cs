...
/*
    ...
    Special Cases: Lightning: Plays a smal flash of lightning over the target dummy.
...
public class StormSchool : MonoBehaviour
{
    [Header("Beam Spell")]
    ...
    public string damageType = "storm";
    ...
    {
        if ((gesture == "CLEAVE" || gesture == "LIGHTNING") && score >= 0.9f)
        {
    ...
    private void OnEnable()
    {
        ...
        airsigManager.onDeveloperDefinedMatch += developerDefined;
        airsigManager.SetDeveloperDefinedTarget(new List<string> { "LIGHTNING", "CLEAVE" });
        airsigManager.SetClassifier("StormGestureProfile", "");
    ...
    void FireBeam(bool gripButtonDown, bool gripButtonUp, bool gripButtonPressed)
    {
        ...
        {
            RaycastHit _hit;
            device1.TriggerHapticPulse(2500);
    ...
    void Update()
    {
        ...
        else if (controllerManager.casting)
        {
            ...
            if (device.GetPressDown(SteamVR_Controller.ButtonMask.Trigger))
            {
                if (lastMatchedGesture == "CLEAVE")
                {
                    CleaveSpell();
                }
                if (lastMatchedGesture == "LIGHTNING")
                {
                    ParticleSystem spellInstance;
                    spellInstance = Instantiate(lightningEffect, targetDummy.transform.position, targetDummy.transform.rotation) as ParticleSystem;
                    lightningSound.Play();
                }
