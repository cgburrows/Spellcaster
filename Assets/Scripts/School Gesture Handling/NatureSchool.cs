...
public class NatureSchool : MonoBehaviour
{
    ...
    //public string damageType = "nature";
    ...
    private void OnEnable()
    {
        ...
        airsigManager.onDeveloperDefinedMatch += developerDefined;
        airsigManager.SetDeveloperDefinedTarget(new List<string> { "HEART", "SNAKE" });
        airsigManager.SetClassifier("NatureGestureProfile", "");
    ...
    void FireBeam(bool gripButtonDown, bool gripButtonUp, bool gripButtonPressed)
    {
        ...
        {
            RaycastHit _hit;
            device1.TriggerHapticPulse(1250);
    ...
    void Update()
    {
        ...
        else if (controllerManager.casting)
        {
            ...
            if (device.GetPressDown(SteamVR_Controller.ButtonMask.Trigger))
            {
                if (lastMatchedGesture == "HEART")
                {
                    ParticleSystem spellInstance;
                    spellInstance = Instantiate(bloomEffect, gameObject.transform.position, gameObject.transform.rotation) as ParticleSystem;
                    bloomSound.Play();
                }
                if (lastMatchedGesture == "SNAKE")
                {
                    CleaveSpell();
                }
