...
/*
    ...
    Special Cases: Freeze Dry: Spawn a short frost effect over the dummy.
    
    For more info on the magic beam, see MagicBeam2.cs.
*/

public class FrostSchool : MonoBehaviour
{
    [Header("Beam Spell")]
    ...
    void HandleOnDeveloperDefinedMatch(long gestureId, string gesture, float score)
    {
        if (score >= 1)
        {
            ...
            if ((gesture == "BOTRN" || gesture == "DOWN") && score >= 0.9f)
            {
    ...
    private void OnEnable()
    {
        ...
        airsigManager.onDeveloperDefinedMatch += developerDefined;
        airsigManager.SetDeveloperDefinedTarget(new List<string> { "BOTRN", "DOWN" });
    ...
    // FREEZE DRY SPELL
    // ===================
    public void FreezeDry(GameObject target)
    {
        Transform fxTarget = target.transform;
        ParticleSystem spellInstance;

        spellInstance = Instantiate(frostEffect, fxTarget) as ParticleSystem;
    }
    // =======================
    ...
    void Update()
    {
        ...
        else if (controllerManager.casting)
        {
            ...
            if (device.GetPressDown(SteamVR_Controller.ButtonMask.Trigger))
            {
                if (lastMatchedGesture == "BOTRN")
                {
                    CleaveSpell();
                }
                if (lastMatchedGesture == "DOWN")
                {
                    ParticleSystem spellInstance;
                    spellInstance = Instantiate(frostEffect, targetDummy.transform.position, targetDummy.transform.rotation) as ParticleSystem;
                    frostSound.Play();
                }
