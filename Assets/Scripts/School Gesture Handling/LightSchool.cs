...
/*
    ...
    Special Cases: Illuminate: Spawns a brilliant particle effect at the tip of the user's wand.
    
    For more info on the magic beam, see MagicBeam2.cs.
*/

public class LightSchool : MonoBehaviour
{
    ...
    [Header("Illuminate Spell")]
    public ParticleSystem illuminateEffect;
    public AudioSource illuminateSound;
    ...
    public string damageType = "light";
    ...
    void HandleOnDeveloperDefinedMatch(long gestureId, string gesture, float score)
    {
        if (score >= 1)
        {
            ...
            if ((gesture == "BOW" || gesture == "C") && score >= 0.9f)
            {
    ...
    private void OnEnable()
    {
        ...
        airsigManager.onDeveloperDefinedMatch += developerDefined;
        airsigManager.SetDeveloperDefinedTarget(new List<string> { "C", "BOW" });
        airsigManager.SetClassifier("LightGestureProfile", "");
    ...
    void FireBeam(bool gripButtonDown, bool gripButtonUp, bool gripButtonPressed)
    {
        ...
        {
            RaycastHit _hit;
            device1.TriggerHapticPulse(250);
    ...
    // CLEAVE SPELL
    // ============
    public void CleaveSpell()
    {
        Vector3 cleavePosition = firePosition.position;
        Quaternion cleaveRotation = firePosition.rotation;

        cleaveRotation.z = 0;
        cleaveRotation.x = 0;
        cleavePosition.y = 1.4f;

        ParticleSystem spellInstance;
        spellInstance = Instantiate(cleavePrefab, cleavePosition, cleaveRotation) as ParticleSystem;
    }
    // ================
    // END CLEAVE SPELL
    ...
    void Update()
    {
        ...
        else if (controllerManager.casting)
        {
            ...
            if (device.GetPressDown(SteamVR_Controller.ButtonMask.Trigger))
            {
                ...
                {
                    ParticleSystem spellInstance;
                    spellInstance = Instantiate(illuminateEffect, firePosition.position, firePosition.rotation) as ParticleSystem;
